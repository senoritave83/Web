...
function verMapa(Lat, Lon, Title, Description)
{
	var wh = window.open("http://www.xmwap.com.br/xmlinkwm/mapa.aspx?lat=" + Lat + "&lon=" + Lon + "&Title=" + Title + "&Description=" + Description, "mapa", "width=600, height=500, toolbars=no")	;
	//wh.focus();

}

function fncOpenMap(vurl, vFeatures) {
    var wh = window.open(vurl, "mapa", "width=600, height=500, toolbars=no");
    if (wh != null)
        wh.focus();
}
...
function fncShowHelp(checkbox, span, message1, message2) {
    if (checkbox != null) {
        if (checkbox.checked) {
            span.innerHTML = message1;
        } else {
            span.innerHTML = message2;
        }
    }
}
