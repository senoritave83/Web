		...
		<asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
				<asp:GridView runat='server' id='GridView1' AutoGenerateColumns='false' AllowSorting='true'>
					...
					<columns>
						<asp:HyperLinkField DataNavigateUrlFields="IDProdutoPesquisa" DataNavigateUrlFormatString="produtopesquisa.aspx?idprodutopesquisa={0}" DataTextField="Codigo" HeaderText="Código" SortExpression="Codigo"  />
                        <asp:BoundField DataField="ProdutoPesquisa" HeaderText="Produto" SortExpression="ProdutoPesquisa"  />
                        <asp:TemplateField HeaderText="Preço Ponta - Mínino" SortExpression="PrecoPontaMinimo" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                            <ItemTemplate>
                                <%# IIf(IsDBNull(Eval("PrecoPontaMinimo")) = True, "", FormatNumber(0 & Eval("PrecoPontaMinimo"), 2))%>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Preço Ponta - Máximo" SortExpression="PrecoPontaMaximo" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                            <ItemTemplate>
                                <%# IIf(IsDBNull(Eval("PrecoPontaMaximo")) = True, "", FormatNumber(0 & Eval("PrecoPontaMaximo"), 2))%>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Preço Varejo - Mínino" SortExpression="PrecoVarejoMinimo" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                            <ItemTemplate>
                                <%# IIf(IsDBNull(Eval("PrecoVarejoMinimo")) = True, "", FormatNumber(0 & Eval("PrecoVarejoMinimo"), 2))%>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Preço Varejo - Máximo" SortExpression="PrecoVarejoMaximo" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                            <ItemTemplate>
