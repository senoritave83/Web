...
Namespace Classes
    ...
    #Region "Metodos"
        ...
        ''' <summary>
        ''' 	Função que retorna uma listagem completa dos grupos do vendedor
        ''' </summary>
        ''' <param name="vReturnType">Tipo de retorno (DataReader ou DataSet)</param>		
        ''' <returns>Retorna um objeto DataReader ou DataSet de acordo com o parâmetro de solicitação</returns>
        ...
        ''' <summary>
        ''' 	Função que retorna uma listagem completa dos grupos do usuário
        ''' </summary>
        ''' <param name="vIDUsuario">Id do Usuário </param>		
        ''' <param name="vMostrarSomenteGruposUsuario">Se true, mostra apenas os grupos pertecentes ao usuário</param>		
        ''' <param name="vReturnType">Tipo de retorno (DataReader ou DataSet)</param>		
        ''' <returns>Retorna um objeto DataReader ou DataSet de acordo com o parâmetro de solicitação</returns>
        Public Function ListarGrupos_Usuario(ByVal vIDUsuario As Integer, Optional ByVal vMostrarSomenteGruposUsuario As Boolean = False, Optional ByVal vReturnType As enReturnType = enReturnType.DataReader) As Object

            Dim cmd As New SqlCommand()
            cmd.CommandText = PREFIXO & "LS_GRUPOS_USUARIOS"
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDUSUARIO", SqlDbType.Int).Value = vIDUsuario
            cmd.Parameters.Add("@MOSTRARSOMENTEGRUPOSUSUARIO", SqlDbType.TinyInt).Value = IIf(vMostrarSomenteGruposUsuario = True, 1, 0)
            Return ExecuteCommand(cmd, vReturnType)
        ...
        ''' <summary>
        ''' 	Rotina que gravar os grupos dos vendedores
        ''' </summary>
        Public Sub GravaGrupoVendedor(ByVal vIDVendedor As Integer, ByVal vIDGrupo As Integer, ByVal vSelected As Boolean)

            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "SV_VENDEDOR_GRUPO"
            cmd.Parameters.Add("@IDVENDEDOR", SqlDbType.Int).Value = vIDVendedor
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDGRUPO", SqlDbType.Int).Value = vIDGrupo
        ...
        ''' <summary>
        ''' 	Rotina que gravar os grupos dos usuários
        ''' </summary>
        Public Sub GravaGrupoUsuario(ByVal vIDUsuario As Integer, ByVal vIDGrupo As Integer, ByVal vSelected As Boolean)

            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "SV_USUARIO_GRUPO"
            cmd.Parameters.Add("@IDUSUARIO", SqlDbType.Int).Value = vIDUsuario
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDGRUPO", SqlDbType.Int).Value = vIDGrupo
            cmd.Parameters.Add("@SELECTED", SqlDbType.Bit).Value = vSelected
