<%@ Page Language="VB" MasterPageFile="~/Principal.master" AutoEventWireup="false" CodeFile="PermissaoAcao.aspx.vb" Inherits="sistema_PermissaoAcao" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <div class='EditArea'>
		<div class='divEditArea'>
			...
			<div class='trField cb' runat='server'  id='trPermissao' visible='true' >
	                        ...
	                        <div class='Acoes'>
	                            <asp:GridView runat='server' AutoGenerateColumns='false' Width="100%" DataSource='<%# ListaAcoes(Eval("Secao")) %>' ShowHeader='false' EnableTheming='false' DataKeyNames='IDAcao' >
	                                <Columns>
	                                    <asp:TemplateField>
	                                        <ItemTemplate>
	                                            <div class='Acao'><asp:CheckBox ID="CheckBox1" Enabled="<%#VerificaPermissaoEdicao()%>" runat="server" Checked='<%#Eval("Selecionado")%>' Text='<%#Eval("Acao")%>' AutoPostBack='true' OnCheckedChanged="chk_OnCheckedChange" /></div>	                                        </ItemTemplate>
	                                    </asp:TemplateField>
	                                </Columns>
	                            </asp:GridView>
	                        </div>
