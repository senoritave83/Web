<%@ Page Language="VB" MasterPageFile="~/Principal.master" AutoEventWireup="false" CodeFile="Bloqueios.aspx.vb" Inherits="Pages.Cadastros.Bloqueios" title="Untitled Page" %>
		...
		<asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
				<asp:GridView runat='server' id='GridView1' AutoGenerateColumns='false' AllowSorting='true'>
					...
					<columns>
						<asp:HyperLinkField DataNavigateUrlFields="IDBloqueio" DataNavigateUrlFormatString="Bloqueio.aspx?IDBloqueio={0}" DataTextField="IDBloqueio" HeaderText="Codigo" SortExpression="IDBloqueio"  />
                        <asp:BoundField DataField="Bloqueio" HeaderText="Bloqueio" SortExpression="Bloqueio" />
                        <asp:TemplateField HeaderText="Permite Pedido" SortExpression="PermitePedido">
                            <ItemTemplate>
                                <%# CDbl(Eval("PermitePedido")).ToString("Não;Sim")%>
                            </ItemTemplate>
                        </asp:TemplateField>
					</columns>
					<EmptyDataTemplate>
					    ...
					    <div class='divEmptyRow'>
							<asp:Label runat='server' ID='lblNaoEncontrados'>
								N&atilde;o h&aacute; Bloqueios com o filtro selecionado!
							</asp:Label>
					    </div>
					</EmptyDataTemplate>
				</asp:GridView>
