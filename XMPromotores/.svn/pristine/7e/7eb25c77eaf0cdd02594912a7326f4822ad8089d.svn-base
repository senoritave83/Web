...
#Region "Metodos"
        ...
        Friend Sub New(ByVal dr As IDataReader)
            If Not dr Is Nothing Then
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaDescontoMax")) Then
                    m_blnUtilizaDescontoMax = False
                Else
                    m_blnUtilizaDescontoMax = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaDescontoMax"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaDescontoMaxObrigatorio")) Then
                    m_blnDescontoMaxObrigatorio = False
                Else
                    m_blnDescontoMaxObrigatorio = dr.GetBoolean(dr.GetOrdinal("SubCategoriaDescontoMaxObrigatorio"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaCorrecao")) Then
                    m_blnUtilizaCorrecao = False
                Else
                    m_blnUtilizaCorrecao = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaCorrecao"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaAprovacaoManual")) Then
                    m_blnUtilizaAprovacaoManual = False
                Else
                    m_blnUtilizaAprovacaoManual = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaAprovacaoManual"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaFaixaVenda")) Then
                    m_blnUtilizaFaixaVenda = False
                Else
                    m_blnUtilizaFaixaVenda = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaFaixaVenda"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaFaixaPesquisa")) Then
                    m_blnUtilizaFaixaPesquisa = False
                Else
                    m_blnUtilizaFaixaPesquisa = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaFaixaPesquisa"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("SubCategoriaUtilizaFaixa")) Then
                    m_blnUtilizaFaixa = False
                Else
                    m_blnUtilizaFaixa = dr.GetBoolean(dr.GetOrdinal("SubCategoriaUtilizaFaixa"))
                End If
            End If
        End Sub
        ...
        Friend Sub SetParameters(ByRef prms As SqlParameterCollection)
            prms.Add("@SUBCATEGORIAUTILIZADESCONTOMAX", SqlDbType.Bit).Value = m_blnUtilizaDescontoMax
            prms.Add("@SUBCATEGORIADESCONTOMAXOBRIGATORIO", SqlDbType.Bit).Value = m_blnDescontoMaxObrigatorio
            prms.Add("@SUBCATEGORIAUTILIZACORRECAO", SqlDbType.Bit).Value = m_blnUtilizaCorrecao
            prms.Add("@SUBCATEGORIAUTILIZAAPROVACAOMANUAL", SqlDbType.Bit).Value = m_blnUtilizaAprovacaoManual
            prms.Add("@SUBCATEGORIAUTILIZAFAIXAVENDA", SqlDbType.Bit).Value = m_blnUtilizaFaixaVenda
            prms.Add("@SUBCATEGORIAUTILIZAFAIXAPESQUISA", SqlDbType.Bit).Value = m_blnUtilizaFaixaPesquisa
        End Sub
