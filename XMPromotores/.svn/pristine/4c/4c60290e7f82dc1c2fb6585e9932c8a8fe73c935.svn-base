...
Partial Class inc_ImageUploader
    ...
    Public Property CausesValidation() As Boolean
        ...
        Set(ByVal value As Boolean)
            ...
            btnSubir.CausesValidation = ViewState("CausesValidation")
            btnSelecionarArquivo.CausesValidation = ViewState("CausesValidation")
        End Set
    End Property
    ...
    Private Function getTela() As Tela
        If SpanEdicao.Visible = True Then
            Return Tela.Edicao
        Else
            Return Tela.Upload
        End If
    ...
    Private Sub MontaTela(ByVal obTela As inc_ImageUploader.Tela)
        Select Case obTela
            Case Tela.Upload
                SpanEdicao.Visible = False
                SpanUpload.Attributes.Remove("style")
                SpanUpload.Visible = True
                'divImagem.Visible = False
            Case Tela.Edicao
                spanEdicao.Visible = True
                SpanUpload.Attributes.Add("style", "display:none;")
    ...
    Private Function getInternetExplorerVersion() As Integer
        Dim rv As Integer = -1 ' Return value assumes failure.
        If Request.Browser.IsBrowser("IE") Then
            rv = Request.Browser.MajorVersion()
        End If
        Return rv
    End Function
    ...
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If getInternetExplorerVersion() >= 9 Then
            Me.btnSelecionarArquivo.Attributes.Add("onclick", "sDiv(" & Me.ClientID & "_SpanUpload);hDiv(" & Me.ClientID & "_SpanEdicao);")
            filImagem.Attributes.Add("onChange", "fncSubir(this);")
            btnVoltar.Attributes.Add("onclick", "hDiv(" & Me.ClientID & "_SpanUpload);sDiv(" & Me.ClientID & "_SpanEdicao);" & btnSubir.ClientID & ".disabled = true;fncTrocaImagem(" & img.ClientID & ", false)")
        Else
            Me.btnSelecionarArquivo.Attributes.Add("onclick", "sDiv(" & Me.ClientID & "_SpanUpload);hDiv(" & Me.ClientID & "_SpanEdicao);document.getElementById('" & filImagem.ClientID & "').click();")
            btnVoltar.Attributes.Add("onclick", "hDiv(" & Me.ClientID & "_SpanUpload);sDiv(" & Me.ClientID & "_SpanEdicao);fncTrocaImagem(" & img.ClientID & ", false)")
            btnVoltar.Attributes.Add("class", "bt_sel_img")
            btnSubir.Style.Add("visibility", "hidden")
            filImagem.Style.Add("visibility", "hidden")
            filImagem.Style.Add("width", "0px")
            filImagem.Style.Add("height", "0px")
            filImagem.Attributes.Add("onChange", "fncSubir(this);")
        End If
    ...
    Protected Overrides Sub Render(ByVal writer As System.Web.UI.HtmlTextWriter)
        ...
        Me.Page.ClientScript.RegisterForEventValidation(btnSubir.UniqueID)
        Me.Page.ClientScript.RegisterForEventValidation(filImagem.UniqueID)
    End Sub
