...
#Region "Declarations"
        ...
        Protected m_bytUnicoPeriodo As Byte
...
#Region "Properties"
        ...
        Public Overridable Property UnicoPeriodo() As Byte
            Get
                Return m_bytUnicoPeriodo
            End Get
            Set(ByVal Value As Byte)
                m_bytUnicoPeriodo = Value
            End Set
        End Property
...
#Region "Metodos"
        ...
        Protected Overridable Sub Inflate(ByVal dr As IDataReader)
            ...
            End If
            If dr.IsDBNull(dr.GetOrdinal("UnicoPeriodo")) Then
                m_bytUnicoPeriodo = 0
            Else
                m_bytUnicoPeriodo = dr.GetByte(dr.GetOrdinal("UnicoPeriodo"))
            End If
        ...
        Public Sub Update()
            ...
            cmd.Parameters.Add("@UNICOPERIODO", SqlDbType.TinyInt).Value = m_bytUnicoPeriodo
        ...
        ''' <summary>
        ''' Função que retorna uma listagem completa de roteiro para exportação
        ''' </summary>
        ''' <param name="vFilter">Filtro</param>
        ''' <param name="vTipoUsuario">Tipo de usuário (filtro)</param>
        ''' <param name="vIDSuperior">Chave primária do Superior (Coordenador e Lider)</param>
        ''' <param name="vUF">UF (filtro)</param>
        ''' <param name="vReturnType">Tipo de retorno (DataReader ou DataSet)</param>		
        ''' <returns>Retorna um objeto DataReader ou DataSet de acordo com o parâmetro passado</returns>
        Public Function Export(ByVal vFilter As String, ByVal vTipoUsuario As Integer, ByVal vIDSuperior As String, ByVal vUF As String, Optional ByVal vReturnType As enReturnType = enReturnType.DataReader) As IDataReader

            Dim arrIDSuperior = Split(vIDSuperior, ",")

            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "LS_EXPORT_ROTEIROS"
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@FILTRO", SqlDbType.VarChar, 100).Value = vFilter
            cmd.Parameters.Add("@IDCOORDENADOR", SqlDbType.Int).Value = CInt(arrIDSuperior(0))
            cmd.Parameters.Add("@IDLIDER", SqlDbType.Int).Value = CInt(arrIDSuperior(1))
        ...
        Public Sub AlterarStatusPesquisa(ByVal vIDLoja As Integer, ByVal vStatusPesquisa As Byte)

            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "UP_ROTEIRO_LOJA_STATUS"
            Dim parameter As New SqlParameter("@RETURN_VALUE", SqlDbType.Int)
            parameter.Direction = ParameterDirection.ReturnValue
            cmd.Parameters.Add(parameter)
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDLOJA", SqlDbType.Int).Value = vIDLoja
            cmd.Parameters.Add("@IDROTEIRO", SqlDbType.Int).Value = m_intIDRoteiro
            cmd.Parameters.Add("@STATUSPESQUISA", SqlDbType.TinyInt).Value = vStatusPesquisa
            ExecuteNonQuery(cmd)

            Me.User.Log("Cadastro de Roteiros", "AlterarStatusPesquisa - IDLOJA=" & vIDLoja & " IDROTEIRO=" & m_intIDRoteiro)
