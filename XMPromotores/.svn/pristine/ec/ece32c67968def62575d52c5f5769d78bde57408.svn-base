...
#Region "Metodos"
        ...
        ''' <summary> 
        ''' 	Função que retorna uma listagem completa dos cargos permitidos na pesquisa
        ''' </summary>
        ''' <returns>Retorna um string contendo todos os id's separados por virgula</returns>
        Public Function CargosPermitidos() As String

            Dim strReturn As String = ""
            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "LS_PESQUISAS_CARGOSPERMITIDOS"
        ...
        ''' <summary>
        ''' 	Função que grava os cargos permitidos na pesquisa
        ''' </summary>
        Public Sub CargosPermitidos_Gravar(ByVal vCargos As String)

            Dim strReturn As String = ""
            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "SV_PESQUISAS_CARGOSPERMITIDOS"
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDPESQUISA", SqlDbType.Int).Value = m_intIDPesquisa
            cmd.Parameters.Add("@CARGOS", SqlDbType.VarChar, 200).Value = vCargos
