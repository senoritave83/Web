...
#End Region
    ...
    #Region "Metodos"
        ...
        ''' 	Função que retorna uma listagem completa
        ''' </summary>
        ''' <param name="vIdMarca">Informa para qual Fornecedor os produtos estão vinculados</param>
        ''' <param name="vReturnType">Tipo de retorno (DataReader ou DataSet)</param>		
        ''' <returns>Retorna um objeto DataReader ou DataSet de acordo com o parâmetro de solicitado</returns>
        Public Function ListarSegmento(ByVal vIdMarca As Integer, Optional ByVal vReturnType As enReturnType = enReturnType.DataReader) As Object

            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "LS_CATEGORIAS_MARCA"
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
        ...
        Public Sub TrocarOrdem(ByVal vIdCategoria As Integer, ByVal vMovimento As Integer)
            ...
            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = PREFIXO & "BS_TROCAORDEM_CATEGORIA"
            cmd.Parameters.Add("@IDEMPRESA", SqlDbType.Int).Value = User.IDEmpresa
            cmd.Parameters.Add("@IDCATEGORIA", SqlDbType.Int).Value = vIdCategoria
