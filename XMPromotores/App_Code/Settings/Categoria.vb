...
#Region "Metodos"
        ...
        Friend Sub New(ByVal dr As IDataReader)
            If Not dr Is Nothing Then
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaDescontoMax")) Then
                    m_blnUtilizaDescontoMax = False
                Else
                    m_blnUtilizaDescontoMax = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaDescontoMax"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaDescontoMaxObrigatorio")) Then
                    m_blnDescontoMaxObrigatorio = False
                Else
                    m_blnDescontoMaxObrigatorio = dr.GetBoolean(dr.GetOrdinal("CategoriaDescontoMaxObrigatorio"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaCorrecao")) Then
                    m_blnUtilizaCorrecao = False
                Else
                    m_blnUtilizaCorrecao = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaCorrecao"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaAprovacaoManual")) Then
                    m_blnUtilizaAprovacaoManual = False
                Else
                    m_blnUtilizaAprovacaoManual = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaAprovacaoManual"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaFaixaVenda")) Then
                    m_blnUtilizaFaixaVenda = False
                Else
                    m_blnUtilizaFaixaVenda = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaFaixaVenda"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaFaixaPesquisa")) Then
                    m_blnUtilizaFaixaPesquisa = False
                Else
                    m_blnUtilizaFaixaPesquisa = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaFaixaPesquisa"))
                End If
                If dr.IsDBNull(dr.GetOrdinal("CategoriaUtilizaFaixa")) Then
                    m_blnUtilizaFaixa = False
                Else
                    m_blnUtilizaFaixa = dr.GetBoolean(dr.GetOrdinal("CategoriaUtilizaFaixa"))
                End If
            End If
        End Sub
        ...
        Friend Sub SetParameters(ByRef prms As SqlParameterCollection)
            prms.Add("@CATEGORIAUTILIZADESCONTOMAX", SqlDbType.Bit).Value = m_blnUtilizaDescontoMax
            prms.Add("@CATEGORIADESCONTOMAXOBRIGATORIO", SqlDbType.Bit).Value = m_blnDescontoMaxObrigatorio
            prms.Add("@CATEGORIAUTILIZACORRECAO", SqlDbType.Bit).Value = m_blnUtilizaCorrecao
            prms.Add("@CATEGORIAUTILIZAAPROVACAOMANUAL", SqlDbType.Bit).Value = m_blnUtilizaAprovacaoManual
            prms.Add("@CATEGORIAUTILIZAFAIXAVENDA", SqlDbType.Bit).Value = m_blnUtilizaFaixaVenda
            prms.Add("@CATEGORIAUTILIZAFAIXAPESQUISA", SqlDbType.Bit).Value = m_blnUtilizaFaixaPesquisa
        End Sub
